...
# 进行迁移学习
def get_mode_GoogleNet(target_num):
    # 引入已经训练好的Resnet模型
    ...
    # 修改其全连接层结构
    GoogleNet.fc = nn.Linear(in_features=1024, out_features=target_num, bias=True)
    return GoogleNet
# 图像处理 数据增强
data_transform = {
...
# 获取已经训练好的模型
googleNet = get_mode_GoogleNet(2)
...
optim = torch.optim.Adam(googleNet.parameters(), lr=learn_s)
...
for i in range(train_round):
    ...
    with torch.no_grad():
        ...
        print(f"准确率为:{acc}")
        if acc > 0.9:
            torch.save(googleNet.state_dict(), os.path.join("../trainmode/VGGMode", f"move_learn_GoogleNet_acc={acc:0.4f}.pth"))
    print("")