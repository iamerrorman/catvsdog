...
# 进行迁移学习
def get_mode_Resnet(target_num):
    # 引入已经训练好的Resnet模型
    ...
    # 修改其全连接层结构
    Resnet50.fc = nn.Linear(in_features=2048, out_features=target_num, bias=True)
    return Resnet50
...
optim = torch.optim.Adam(resnet50.parameters(), lr=learn_s)
...
for i in range(train_round):
    ...
    with torch.no_grad():
        ...
        print(f"准确率为:{acc}")
        if acc > 0.9:
            torch.save(resnet50.state_dict(), os.path.join("../trainmode/VGGMode", f"move_learn_Resnet_acc={acc:0.4f}.pth"))
    print("")