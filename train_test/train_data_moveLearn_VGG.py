...
optim = torch.optim.Adam(VGG16.parameters(), lr=learn_s)
...
for i in range(train_round):
    ...
    with torch.no_grad():
        ...
        print(f"准确率为:{acc}")
        if acc > 0.60:
            torch.save(VGG16.state_dict(), os.path.join("../trainmode/VGGMode", f"acc={acc:0.4f}.pth"))
    print("")